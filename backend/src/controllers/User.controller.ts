...
class UserController extends Controller {
	...
	async create(req: Request, res: Response) {
		...
		try {
			...
			const userByWallet = await this.service.fetchByWalletAddress(body.wallet_address);
			if (userByWallet !== null) throw Error(getErrorMessage('walletAddressAlreadyExist'));
	...
	async fetchIsWalletAddressUnique(req: Request, res: Response) {
		try {
			const isUnique = (await this.service.fetchByWalletAddress(req.params.wallet_address)) === null;
